...
async function waitForSessionReady(maxAttempts = 10, delayMs = 1200) {
...
export async function signIn(email, password) {
    try {
        ...
        // ✅ INTEGRATED SAFARI POLLING LOGIC
        const baseDelay = isSafari ? 3000 : 1500;
        const maxAttempts = isSafari ? 25 : 15;
        
        console.log(isSafari 
            ? '🧭 Safari detected - using extended polling' 
            : '🌐 Standard browser - using normal polling'
        );

        // We use your helper function here to verify the session is actually set
        const sessionActive = await waitForSessionReady(maxAttempts, baseDelay);
        
        if (!sessionActive) {
            throw new Error('Session verification timeout - cookie not detected');
        }
